...
int arImageProcLumaHist(ARImageProcInfo *ipi,
    ...
    vImage_Error err;
    vImage_Buffer buf = {
        (void *)dataPtr, ipi->imageY, ipi->imageX, ipi->imageX};
    if ((err = vImageHistogramCalculation_Planar8(&buf, ipi->histBins, 0)) !=
        kvImageNoError) {
        ARLOGe("arImageProcLumaHist(): vImageHistogramCalculation_Planar8 "
               "error %ld.\n",
               err);
        return (-1);
    }
    ...
    for (p = (unsigned char *__restrict)dataPtr;
         p < dataPtr + ipi->imageX * ipi->imageY;
         p++)
        ipi->histBins[*p]++;
...
unsigned char *arImageProcGetHistImage(ARImageProcInfo *ipi) {
    ...
    unsigned char *buf =
        (unsigned char *)calloc(1, 256 * 256 * sizeof(unsigned char));
    if (buf) {
        for (i = 0; i < 256; i++)
            if (ipi->histBins[i] > peak)
                peak = ipi->histBins[i]; // Find value of mode.
        scalef = 256.0f / (float)peak;
        for (i = 0; i < 256; i++) {
            y = (int)((float)ipi->histBins[i] * scalef);
            if (y > 256)
                y = 256; // Safety in case of FP rounding errors etc.
            for (j = 0; j < y; j++)
                buf[256 * j + i] = 0xff;
        }
    }
...
int arImageProcLumaHistAndCDFAndPercentile(ARImageProcInfo *ipi,
    ...
    i = 0;
    while (ipi->cdfBins[i] < requiredCD)
        i++; // cdfBins[i] >= requiredCD
    j = i;
    while (ipi->cdfBins[j] == requiredCD)
        j++; // cdfBins[j] > requiredCD
...
int arImageProcLumaHistAndCDFAndMedian(ARImageProcInfo *ipi,
                                       ...
                                       unsigned char *value_p) {
    return (
        arImageProcLumaHistAndCDFAndPercentile(ipi, dataPtr, 0.5f, value_p));
}
...
int arImageProcLumaHistAndBoxFilterWithBias(ARImageProcInfo *ipi,
                                            const ARUint8 *__restrict dataPtr,
                                            const int boxSize, const int bias) {
    int ret, i;
#if !AR_IMAGEPROC_USE_VIMAGE
    ...
    if (!ipi->image2) {
        ipi->image2 = (unsigned char *)malloc(ipi->imageX * ipi->imageY *
    ...
    if (!ipi->tempBuffer) {
        ...
        if ((err = vImageBoxConvolve_Planar8(&src,
                                             &dest,
                                             NULL,
                                             0,
                                             0,
                                             ...
                                             kvImageTruncateKernel |
                                                 kvImageGetTempBufferSize)) < 0)
            return (-1);
        if (!(ipi->tempBuffer = malloc(err)))
            return (-1);
    }
    err = vImageBoxConvolve_Planar8(&src,
                                    &dest,
                                    ipi->tempBuffer,
                                    0,
                                    0,
    ...
    for (j = 0; j < ipi->imageY; j++) {
        for (i = 0; i < ipi->imageX; i++) {
            ...
            val = count = 0;
            for (kernel_j = -kernelSizeHalf; kernel_j <= kernelSizeHalf;
                 kernel_j++) {
                jj = j + kernel_j;
                if (jj < 0 || jj >= ipi->imageY)
                    continue;
                for (kernel_i = -kernelSizeHalf; kernel_i <= kernelSizeHalf;
                     kernel_i++) {
                    ii = i + kernel_i;
                    if (ii < 0 || ii >= ipi->imageX)
                        continue;
...
#endif
    if (bias)
        for (i = 0; i < ipi->imageX * ipi->imageY; i++)
            ipi->image2[i] += bias;
    return (0);
}
...
int arImageProcLumaHistAndCDFAndLevels(ARImageProcInfo *ipi,
    ...
    l = 0;
    while (ipi->cdfBins[l] == 0)
        l++;
    ipi->min = l;
    maxCD = ipi->imageX * ipi->imageY;
    while (ipi->cdfBins[l] < maxCD)
        l++;
    ipi->max = l;
